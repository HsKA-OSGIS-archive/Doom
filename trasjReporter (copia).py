...
def login():
    ...
    if request.method == 'POST':
        ...
        if sha256_crypt.verify(password_candidate, data):
            ...
            d_conn = pg_operations2.pg_disconnect2(d_conn)
            return redirect(url_for('map', email = session['email']))
        else:
            error = 'Invalid login'
...
def checkPol(lat, lon, form):
    ...
    conn=d_conn['conn']
    cursor=d_conn['cursor'] 
    cursor.execute("SELECT ST_Extent(geom) as bextent FROM bbox")
    # if changing to points bbox
    #cursor.execute("select min(st_y(geom)),max(st_y(geom)),min(st_x(geom)),max(st_x(geom)) from bbox")
    data = cursor.fetchall()
    for row in data[0]:
        print row
        minX =  row[0]
